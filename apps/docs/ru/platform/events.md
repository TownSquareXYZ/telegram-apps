...
| Поле | Тип      | Описание                                                       |
| ---- | -------- | -------------------------------------------------------------- |
| x    | `number` | Текущее ускорение по оси X, измеряемое в м/с². |
| y    | `number` | Текущее ускорение по оси Y, измеряемое в м/с². |
| z    | `number` | Текущее ускорение по оси Z, измеряемое в м/с². |

### `accelerometer_failed`

Доступно с **версии 8.0**

Не удалось запустить отслеживание данных акселерометра.
...
Начато отслеживание данных акселерометра.

### `accelerometer_stopped`

Доступно с **версии 8.0**

Отслеживание данных акселерометра остановлено.
...
Изменились данные об ориентации устройства.

| Поле     | Тип       | Описание                                                                                                                                                |
| -------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| absolute | `boolean` | _Опционально_.  Нулевое значение, указывающее, предоставляет ли устройство данные об ориентации в абсолютных значениях. |
| alpha    | `number`  | Вращение вокруг оси Z, измеряемое в радианах.                                                                                           |
| beta     | `number`  | Вращение вокруг оси X, измеряемое в радианах.                                                                                           |
| gamma    | `number`  | Вращение вокруг оси Y, измеряемое в радианах.                                                                                           |

### `device_orientation_failed`

Доступно с **версии 8.0**

Не удалось запустить отслеживание данных об ориентации устройства.
...
Начато отслеживание данных об ориентации устройства.

### `device_orientation_stopped`

Доступно с **версии 8.0**

Отслеживание данных об ориентации устройства остановлено.
...
Изменились данные гироскопа.

| Поле | Тип      | Описание                                                                    |
| ---- | -------- | --------------------------------------------------------------------------- |
| x    | `number` | Текущая скорость вращения вокруг оси X, измеряемая в рад/с. |
| y    | `number` | Текущая скорость вращения вокруг оси Y, измеряемая в рад/с. |
| z    | `number` | Текущая скорость вращения вокруг оси Z, измеряемая в рад/с. |

### `gyroscope_failed`

Доступно с версии: **8.0**

Не удалось выполнить отслеживание данных гироскопа.
...
Начато отслеживание данных гироскопа.

### `gyroscope_stopped`

Доступно с **версии 6.1**

Отслеживание данных гироскопа остановлено.
...
Был получен статус доступности функций, связанных с местоположением.

| Поле                                  | Тип       | Описание                                                                                     |
| ------------------------------------- | --------- | -------------------------------------------------------------------------------------------- |
| available                             | `boolean` | Показывает, доступно ли отслеживание местоположения.                         |
| access_requested | `boolean` | Показывает, был ли запрошен разрешение на отслеживание местоположения.       |
| access_granted   | `boolean` | Показывает, было ли предоставлено разрешение на отслеживание местоположения. |

### `location_requested`

Доступно с версии: **8.0**

Приложение получило информацию о текущем местоположении пользователя.

| Поле                                     | Тип       | Описание                                                                                                                                                                          |
| ---------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| available                                | `boolean` | Показывает, доступно ли отслеживание местоположения.                                                                                                              |
| latitude                                 | `number`  | Широта в градусах. Устанавливается только если `available` имеет значение True.                                                                   |
| longitude                                | `number`  | Долгота в градусах. Устанавливается только если `available` имеет значение True.                                                                  |
| altitude                                 | `number`  | _Опционально_. Высота над уровнем моря в метрах. Устанавливается только если `available` имеет значение True.                     |
| course                                   | `number`  | _Опционально_. Направление движения устройства в градусах. Устанавливается только если `available` имеет значение True.           |
| speed                                    | `number`  | _Необязательно_. Скорость устройства в м/с. Устанавливается только если `available` имеет значение True.                          |
| horizontal_accuracy | `number`  | _Необязательно_. Точность значений широты и долготы в метрах. Устанавливается только если `available` имеет значение True.        |
| vertical_accuracy   | `number`  | _Необязательно_. Точность значения высоты над уровнем моря в метрах. Устанавливается только если `available` имеет значение True. |
| course_accuracy     | `number`  | _Необязательно_. Точность значения курса в градусах. Устанавливается только если `available` имеет значение True.                 |
| speed_accuracy      | `number`  | _Необязательно_. Точность значения скорости в м/с. Устанавливается только если `available` имеет значение True.                   |
...
| Поле                           | Тип      | Описание                                                                                                                                                                                     |
| ------------------------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| button_id | `string` | _Необязательно_. Идентификатор нажатой кнопки. Если всплывающее окно было закрыто без нажатия какой-либо кнопки, это свойство будет опущено. |

### `prepared_message_failed`

Доступно с версии: **8.0**

Не удалось отправить подготовленное сообщение.
...
Было отправлено подготовленное сообщение.
...
| Поле | Тип      | Описание                                                                   |
| ---- | -------- | -------------------------------------------------------------------------- |
| data | `string` | _Необязательно_. Данные, полученные из QR. |
...
| width                                                     | `number`  | _Необязательно_. Ширина области просмотра.                                          |
