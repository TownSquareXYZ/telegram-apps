...
Запрашивает состояние доступности функций, связанных с местоположением.
...
Открывает настройки доступа к местоположению для ботов. Полезно, когда вам нужно запросить доступ к местоположению
у пользователей, которые еще не предоставили его.
...
Отображает всплывающее окно, предлагающее пользователю загрузить файл.
...
Запрашивает данные о местоположении.
...
Открывает диалоговое окно, позволяющее пользователю поделиться сообщением, отправленным ботом.

| Поле | Тип      | Описание                                                                                                                                                                                                                                                                              |
| ---- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| id   | `string` | Идентификатор сообщения ([PreparedInlineMessage](https://core.telegram.org/bots/api#preparedinlinemessage)), ранее полученного с помощью метода Bot API [savePreparedInlineMessage](https://core.telegram.org/bots/api#savepreparedinlinemessage). |
...
| has_shine_effect    | `boolean` | _Необязательный_. Должна ли кнопка иметь эффект сияния.                                                                                                                                                                     | `v7.8`     |
...
Начинает отслеживать данные акселерометра.

| Поле                              | Тип      | Описание                                                                                                                                                                                                                                                                   |
| --------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| refresh_rate | `number` | Частота обновления в миллисекундах, допустимые значения от 20 до 1000. Обратите внимание, что `refresh_rate` может поддерживаться не на всех платформах, поэтому фактическая частота отслеживания может отличаться от указанного значения. |

### `web_app_start_device_orientation`

Доступно с **версии 6.1**

Начинает отслеживать данные об ориентации устройства.

| Поле                               | Тип       | Описание                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| refresh_rate  | `number`  | Частота обновления в миллисекундах, допустимые значения от 20 до 1000. Обратите внимание, что `refresh_rate` может поддерживаться не на всех платформах, поэтому фактическая частота отслеживания может отличаться от указанного значения.                                                                                                                                                                                                                                                                                                                                                                          |
| need_absolute | `boolean` | _Необязательный_. Укажите значение true для получения данных об абсолютной ориентации, позволяющих определить положение устройства относительно северного магнитного полюса. Используйте этот параметр при внедрении в ваше приложение таких функций, как компас. Если относительных данных достаточно, передайте значение false. <br/><br/> Помните, что некоторые устройства могут не поддерживать данные об абсолютной ориентации. В таких случаях вы получите относительные данные, даже если передано need_absolute=true. |

### `web_app_start_gyroscope`

Доступно с **версии 8.0**

Начинает отслеживать данные гироскопа.
...
Прекращает отслеживание данных об ориентации устройства.

### `web_app_stop_gyroscope`

Доступно с **версии 8.0**

Прекращает отслеживание данных гироскопа.
...
<table>
  ...
  <tr>
    ...
    <td>
      <i>Опционально</i>. Отображать ли индикатор загрузки внутри кнопки. Используйте это свойство, если 
...
Фиксирует ориентацию мини-приложения в его текущем режиме (книжном или альбомной ориентации). После блокировки ориентация остается фиксированной, независимо от поворота устройства. Это полезно, если требуется стабильная ориентация во время определенных взаимодействий.

| Поле   | Тип       | Описание                                                         |
| ------ | --------- | ---------------------------------------------------------------- |
| locked | `boolean` | True, если ориентация должна быть заблокирована. |
