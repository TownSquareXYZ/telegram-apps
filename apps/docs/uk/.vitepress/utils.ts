...
function section(text: string, items: Items, prefix: string) {
  return {
    text,
    items: itemsToArray(items, prefix)
  };
}
...
export const linkGenerator = (prefix: string, navItem: navItem, linkJson: LinkJson) => {
  ...
  const prefixSideBar = {
    [prefix]: Object.entries(linkJson).map(([key, value]: [string, Items]) => section(key, value))
  };
  return {
    prefixNavItem,
    prefixSideBar
